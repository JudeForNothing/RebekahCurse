...
function yandereWaifu.RebekahGoldBarrage(player, direction)
	...
	if current1Neds < 8 then
		for i = 1, nedClones do
			...
			if rng >= 50 and rng <= 100 and player:HasCollectible(CollectibleType.COLLECTIBLE_SPIRIT_SWORD) then
				subtype = 5
			elseif rng >= 40 and rng <= 100 and player:HasCollectible(CollectibleType.COLLECTIBLE_DR_FETUS) or  player:HasCollectible(CollectibleType.COLLECTIBLE_EPIC_FETUS) then
				subtype = 2
			elseif rng >= 30 and rng <= 100 and player:HasCollectible(CollectibleType.COLLECTIBLE_MOMS_KNIFE) then
				subtype = 1
			elseif rng >= 20 and rng <= 100 and (player:HasCollectible(CollectibleType.COLLECTIBLE_TECHNOLOGY) or player:HasCollectible(CollectibleType.COLLECTIBLE_TECH_X)) then
				subtype = 3
			elseif rng >= 10 and rng <= 100 and player:HasCollectible(CollectibleType.COLLECTIBLE_BRIMSTONE) then
				subtype = 4
			end
			local ned = Isaac.Spawn( EntityType.ENTITY_FAMILIAR, RebekahCurse.ENTITY_NED_NORMAL, subtype, player.Position, Vector( 0, 0 ), player):ToFamiliar();
	...
	for n, ned in pairs( Isaac.FindByType(EntityType.ENTITY_FAMILIAR, -1, -1, false, false) ) do
		if GetPtrHash(ned:ToFamiliar().Player:ToPlayer()) == GetPtrHash(player) then
			if ned.Variant == RebekahCurse.ENTITY_NED_NORMAL then
				...
				local crack = Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.CRACK_THE_SKY, 1, ned.Position, Vector(0,0), player) 
	...
	if player.Luck + math.random(1,10) >= 8 and player:HasCollectible(CollectibleType.COLLECTIBLE_LOKIS_HORNS) then
		for i = 1, 4 do
			local ned = Isaac.Spawn( EntityType.ENTITY_FAMILIAR, RebekahCurse.ENTITY_NED_NORMAL, 10, player.Position, Vector( 0, 0 ), player):ToFamiliar();
		end
	end
	if player.Luck + math.random(1,10) >= 5 and player:HasCollectible(CollectibleType.COLLECTIBLE_MOMS_EYE) then
		local ned = Isaac.Spawn( EntityType.ENTITY_FAMILIAR, RebekahCurse.ENTITY_NED_NORMAL, 11, player.Position, Vector( 0, 0 ), player):ToFamiliar();
	end
	if player:HasCollectible(CollectibleType.COLLECTIBLE_MONSTROS_LUNG) then
		for i = 1, math.random(3,5) do
			local ned = Isaac.Spawn( EntityType.ENTITY_FAMILIAR, RebekahCurse.ENTITY_NED_NORMAL, 12, player.Position, Vector( 0, 0 ), player):ToFamiliar();
		end
	end
...
function yandereWaifu:onFamiliarNedInit(fam)
	...
	elseif fam.SubType == 2 then --dr fetus
		sprite:ReplaceSpritesheet(0, "gfx/effects/gold/bombs/familiar_ned.png") 
		sprite:ReplaceSpritesheet(1, "gfx/effects/gold/bombs/familiar_ned.png") 
		...
		for k, enemy in pairs( Isaac.GetRoomEntities() ) do
			if enemy:IsVulnerableEnemy() then
				if enemy.Position:Distance( eff.Position ) < enemy.Size + eff.Size + 5 then
					...
					--if targetAngle >= (angleNum - 90) and targetAngle <= (angleNum + 90) then
						enemy:TakeDamage(data.Damage or 0.4, 0, EntityRef(eff), 4)
					--end
				end
			end
		end
...
yandereWaifu:AddCallback(ModCallbacks.MC_FAMILIAR_UPDATE, function(_,  fam) --christian nerd
	...
	elseif spr:IsFinished("Charge") then
		...
		if fam.SubType ~= 5 and fam.SubType ~= 3 then
	...
	elseif spr:IsPlaying("RocketJump") then
		if spr:GetFrame() == 9 then
			Isaac.Explode(fam.Position, fam.Player, 10)
		end
	...
	if spr:IsPlaying("Idle") then
		...
		for i, e in pairs(Isaac.GetRoomEntities()) do
			...
			if e.Type ~= EntityType.ENTITY_PLAYER then
				if e:IsActiveEnemy() then
					if e:IsVulnerableEnemy() then
						if fam.FrameCount % 120 == 0 and fam.SubType ~= 2 then
							local rng2 = 1
							if fam.SubType == 4 then
	...
	elseif spr:IsPlaying("Throw") then
		if spr:GetFrame() == 20 then
			...
			elseif fam.SubType == 4 then
				...
				yandereWaifu.GetEntityData(beam).BySquire = true
				yandereWaifu.GetEntityData(beam).Damage = 1.5
			else --normal spear
