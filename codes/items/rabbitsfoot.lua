...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_PICKUP_INIT, function(_, pickup)
	...
	for p = 0, ILIB.game:GetNumPlayers() - 1 do
		...
		if player:HasTrinket(RebekahCurse.TRINKET_RABBITSFOOT) or isEaster then
			...
			print(chance)
			--pickup.Wait = 10;
			local validPickup = (pickup.Variant == PickupVariant.PICKUP_HEART or pickup.Variant == PickupVariant.PICKUP_COIN or pickup.Variant == PickupVariant.PICKUP_KEY or pickup.Variant == PickupVariant.PICKUP_BOMB or pickup.Variant == PickupVariant.PICKUP_CHEST or pickup.Variant == PickupVariant.PICKUP_PILL)
			if rng:RandomFloat() <= (chance) and validPickup and ILIB.room:IsFirstVisit() then
