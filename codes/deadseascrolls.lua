local dssmod = RegisterMod("Dead Sea Scrolls (Cursed and Heartbroken)", 1)
local game = Game()
...
-- Cursed and Heartbroken's menu code starts here
function MenuProvider.SaveSaveData()
    MoreCursors.SaveSaveData()
end

function MenuProvider.GetPaletteSetting()
    return MoreCursors.savedata.MenuPalette
end

function MenuProvider.SavePaletteSetting(var)
    MoreCursors.savedata.MenuPalette = var
end

function MenuProvider.GetHudOffsetSetting()
    return MoreCursors.savedata.HudOffset
end

function MenuProvider.SaveHudOffsetSetting(var)
    MoreCursors.savedata.HudOffset = var
end

function MenuProvider.GetGamepadToggleSetting()
    return MoreCursors.savedata.MenuControllerToggle
end

function MenuProvider.SaveGamepadToggleSetting(var)
    MoreCursors.savedata.MenuControllerToggle = var
end

function MenuProvider.GetMenuKeybindSetting()
    return MoreCursors.savedata.MenuKeybind
end

function MenuProvider.SaveMenuKeybindSetting(var)
    MoreCursors.savedata.MenuKeybind = var
end

function MenuProvider.GetMenusNotified()
    return MoreCursors.savedata.MenusNotified
end

function MenuProvider.SaveMenusNotified(var)
    MoreCursors.savedata.MenusNotified = var
end

function MenuProvider.GetMenusPoppedUp()
    return MoreCursors.savedata.MenusPoppedUp
end

function MenuProvider.SaveMenusPoppedUp(var)
    MoreCursors.savedata.MenusPoppedUp = var
end
...
if not dssmenu or (dssmenu.CoreVersion < dssCoreIncluded) then
    ...
    dssmenu.CoreMod = "Cursed and Heartbroken"
    DeadSeaScrollsMenu = dssmenu
end

function MenuProvider.IsMenuCore()
    return DeadSeaScrollsMenu.CoreMod == "Cursed and Heartbroken"
end
...
local cursordirectory = {
    ...
    settings = {
        ...
        buttons = {
            hudOffsetButton,
            gamepadToggleButton,
            menuKeybindButton, --might need this later
            {
                str = "cursor gfx",
                choices = {},
                variable = "cursor",
                ...
                load = function()
                    return MoreCursors.savedata.cursor
                end,
                store = function(var)
                    MoreCursors.savedata.cursor = var

                    local cursor = MoreCursors.Cursors[var]

                    MoreCursors.CursorSprite:Load(cursor[3], true)

                    MoreCursors.CursorSprite:ReplaceSpritesheet(0, cursor[2])
                    MoreCursors.CursorSprite:LoadGraphics()

                    MoreCursors.SaveSaveData()
                end,

                tooltip = {
                    spr = {sprite = MoreCursors.ForRender[1] or Sprite(), width = 16, height = 16, scale = Vector(1, 1), center = true},
                    pos = Vector(8, 8),
                },
            },

            {
                ...
                load = function()
                    return MoreCursors.savedata.scale
                end,
                store = function(var)
                    MoreCursors.savedata.scale = var
...

            {
                ...
                load = function()
                    return MoreCursors.savedata.random and 1 or 2
                end,
                store = function(var)
                    MoreCursors.savedata.random = (var == 1)

                    MoreCursors.SaveSaveData()
                end,

                tooltip = {strset = {"should cursor", "gfx randomise", "each room"}}
            },
        },

        postrender = function(item)
            local button = item.buttons[item.bsel]

            if button and button.variable == "cursor" and MoreCursors.ForRender then
                local key = button.variable
                local setting = button.setting

                local sprite = MoreCursors.ForRender[setting]
                button.tooltip.spr.sprite = sprite
            end
        end,
    },
    credits = {
...
MoreCursors.Directory = cursordirectory

local credits = {
    {"xalum",               "code, sprites"},
    {"peas",                "idea, sprites"},
    {"fuyucchi (maria)",    "sprites"},
    {"pkpseudo",            "sprites"},
    {"sbody",               "spirtes"},
}
...
for _, credit in ipairs(credits) do
    if type(credit) == "string" then
        cursordirectory.credits.buttons[#cursordirectory.credits.buttons + 1] = {str = credit, nosel = true}
    else
        for i, part in ipairs(credit) do
            if i ~= 1 then
                if i == 2 then
                    ...
                    cursordirectory.credits.buttons[#cursordirectory.credits.buttons + 1] = button
                else
                    cursordirectory.credits.buttons[#cursordirectory.credits.buttons + 1] = {strpair = {{str = ''}, {str = part}}, nosel = true}
                end
            end
        end
    end
...
local cursordirectorykey = {
    Item = cursordirectory.main,
    Main = 'main',
    Idle = false,
    MaskAlpha = 1,
    Settings = {},
...
dssmenu.AddMenu("More Cursors", {Run = dssmod.runMenu, Open = dssmod.openMenu, Close = dssmod.closeMenu, Directory = cursordirectory, DirectoryKey = cursordirectorykey})