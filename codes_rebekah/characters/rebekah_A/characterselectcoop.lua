...
local technical_character = RebekahCurse.TECHNICAL_REB 
local maxplayer = #yandereWaifu.ListOfRegPersonalities
...
function yandereWaifu:ShowRebekahCoopMenu(player) --Activates when Technical character is not the first player
...
function yandereWaifu:ChangePlayerTypeToRebekahCoop(player, currentPlayer)
...
function yandereWaifu:render(player) --function that activates the co op menu after Technical character spawns
	...
	and player_type == technical_character then
		...
		if Input.IsActionTriggered(ButtonAction.ACTION_MENUCONFIRM,player.ControllerIndex) and data.ClickPastFirstFrame then
				...
				data.ClickPastFirstFrame=nil
				yandereWaifu:ChangePlayerTypeToRebekahCoop(player, yandereWaifu.ListOfRegPersonalities[data.RebekahCurrentPersonality].playerId)
		...
		coopmenu:SetFrame(0)
		coopmenu:ReplaceSpritesheet(0, yandereWaifu.ListOfRegPersonalities[data.RebekahCurrentPersonality].coopGraphics or "gfx/ui/coop/icons/red.png")
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_PLAYER_RENDER,yandereWaifu.render)
