...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
	if eff.SubType == 6 then
		...
		if eff.FrameCount % 3 == 0 then
			local eff2 = Isaac.Spawn(EntityType.ENTITY_EFFECT, 2, 3, eff.Position + Vector(0,-40), Vector(0,0), player) --vault effect
			eff2.DepthOffset = 400
		end
	end
		...
		if sprite:GetFrame() == 5 then
			for i, ent in pairs (Isaac.GetRoomEntities()) do
				if (ent:IsEnemy() and ent:IsVulnerableEnemy()) or ent.Type == EntityType.ENTITY_FIREPLACE and not ent:IsDead() then
					...
					if ent.Position:Distance((eff.Position)+ (Vector(8,0):Rotated(data.PermanentAngle))) <= 90 + additDistance then
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
	--for i,player in ipairs(ILIB.game:GetNumPlayers()-1) do
		...
		elseif sprite:IsPlaying("Swing") then
			if sprite:GetFrame() >= 2 and sprite:GetFrame() <= 4 then
				local gridPos = 0
				local step = 0 --way to push the hitbox further, idk how the better method is
				if sprite:GetFrame() == 5 then
					step = 12
				...
				for i, ent in pairs (Isaac.GetRoomEntities()) do
					if (ent:IsEnemy() and ent:IsVulnerableEnemy()) or ent.Type == EntityType.ENTITY_FIREPLACE and not ent:IsDead() then
						...
						if ent.Position:Distance((eff.Position)+ (Vector(65 + step,0):Rotated(data.PermanentAngle - 90))) <= 110 + additDistance then
							ent:TakeDamage((player.Damage)*dmg, 0, EntityRef(eff), 1)
							--bleed chance
							if math.random(1,3) == 3 and data.IsJockeySpear then --can only do the bleed if on the jockey
								ent:AddEntityFlags(EntityFlag.FLAG_BLEED_OUT)
...
function yandereWaifu:onFamiliarBoneJockeyInit(fam)
	...
	if fam.SubType == 0 then
		if player:HasCollectible(CollectibleType.COLLECTIBLE_DR_FETUS) then
			...
			sprite:ReplaceSpritesheet(2, "gfx/effects/bone/corpseeater/corpse_eater_rider"..tostring(skincolor)..".png")
			sprite:ReplaceSpritesheet(3, "gfx/effects/bone/corpseeater/corpse_eater_rider"..tostring(skincolor)..".png")
			sprite:ReplaceSpritesheet(4, "gfx/effects/bone/corpseeater/corpse_hair.png")
			sprite:LoadGraphics()
		end
	...
	if fam.SubType == 0 then
		...
		if not data.IsPossessed then
			...
			if (shootingInput.X ~= 0 or shootingInput.Y ~= 0) then
				...
				local firedelay = math.ceil(player.MaxFireDelay/2)
				if player:HasWeaponType(WeaponType.WEAPON_BRIMSTONE) then
					firedelay = math.ceil(player.MaxFireDelay*1.5)
				elseif player:HasWeaponType(WeaponType.WEAPON_FETUS) then
					firedelay = math.ceil(player.MaxFireDelay*3)
				end
				
				if fam.FrameCount % (firedelay) == 0 then
					if player:HasWeaponType(WeaponType.WEAPON_BRIMSTONE) then
						local beam = player:FireBrimstone( shootingInput, fam, 2):ToLaser();
						beam.Position = fam.Position
						yandereWaifu.GetEntityData(beam).BrimBone = true
						beam.CollisionDamage = player.Damage * 0.5
					elseif player:HasWeaponType(WeaponType.WEAPON_FETUS) then
						local leech2 = Isaac.Spawn(EntityType.ENTITY_FAMILIAR, RebekahCurse.ENTITY_BONEJOCKEY, 10, player.Position, Vector(0,0), player):ToFamiliar()
						yandereWaifu.GetEntityData(leech2).ParentLeech = leech
						yandereWaifu.GetEntityData(leech2).DeathFrame = 120
						leech:ClearEntityFlags(EntityFlag.FLAG_APPEAR)
					else
						local tears = player:FireTear(fam.Position, shootingInput:Resized(9), false, false, false):ToTear()
						tears.Position = fam.Position
			...
			if player:HasWeaponType(WeaponType.WEAPON_SPIRIT_SWORD) then
				if player:HasCollectible(CollectibleType.COLLECTIBLE_MOMS_KNIFE) then
					if not data.JockeySpear or data.JockeySpear:IsDead() then
						local spear = Isaac.Spawn(EntityType.ENTITY_EFFECT, RebekahCurse.ENTITY_BONESPEAR, 0, player.Position, Vector(0,0), player):ToEffect();
					...
					if (shootingInput.X ~= 0 or shootingInput.Y ~= 0) and fam.FrameCount % 5 == 0 then
						local cut = Isaac.Spawn(EntityType.ENTITY_EFFECT, RebekahCurse.ENTITY_BONEPUNCH, 0, player.Position, Vector(0,0), player):ToEffect();
						yandereWaifu.GetEntityData(cut).PermanentAngle = shootingInput:GetAngleDegrees()
						yandereWaifu.GetEntityData(cut).Player = player
						--yandereWaifu.GetEntityData(cut).Pos = bonePos
						cut:GetSprite():Load("gfx/effects/bone/melee/punch_sword.anm2", true)
						if player:HasCollectible(CollectibleType.COLLECTIBLE_DEATHS_TOUCH) then
							cut:GetSprite():Load("gfx/effects/bone/melee/punch_scythe.anm2", true)
						end
					end
			...
			if data.JockeySpear then
				...
				if (shootingInput.X ~= 0 or shootingInput.Y ~= 0) then
					--yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = shootingInput:GetAngleDegrees() + 90
					...
					--yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = data.lastFireDir:GetAngleDegrees() + 90
					if data.SpearCharge >= data.MaxSpearCharge then
						...
						if player:HasWeaponType(WeaponType.WEAPON_SPIRIT_SWORD) then
							if not data.DidSwing then
								if not data.JockeySpear:GetSprite():IsPlaying("Swing") then
									data.JockeySpear:GetSprite():Play("Swing", true)
								end
								data.DidSwing = true
							end
						end
				...
				--local shootInputDegree = shootingInput:GetAngleDegrees() + 90
				local correctedLastFireDir = data.lastFireDir:GetAngleDegrees() + 90
				local currentAngle = yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle
				--print(yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle)
				if not currentAngle or currentAngle ~= correctedLastFireDir then
					--[[local targetAnglePos = (currentAngle+correctedLastFireDir)
					local targetAnglePosNeg = currentAngle-correctedLastFireDir
					print(math.abs(targetAnglePos))
					print(math.abs(targetAnglePosNeg))
					if math.abs(targetAnglePosNeg) < math.abs(targetAnglePos) then --rotating shenanigans
						yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle - 15
					else
						yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle + 15
					end]]
					--[[if (correctedLastFireDir <= 0 or yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle > -90) and (correctedLastFireDir ~= 0 and data.lastFireDir:GetAngleDegrees() < 180) then --rotating shenanigans
						yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle - 15
					else]]
					--	yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle + 15
					--end
					yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = correctedLastFireDir
				end
				if yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle >= 360 then yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = 0 end
				if yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle < 0 then yandereWaifu.GetEntityData(data.JockeySpear).PermanentAngle = 360 end
			end
