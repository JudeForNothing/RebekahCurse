...
--zamiel's code
function InutilLib.SpawnTrail(player, color, customPlace)
	local c = color or Color(1, 0, 1, 0.5) -- sets the color of the trail
	local trail = Isaac.Spawn(EntityType.ENTITY_EFFECT, EffectVariant.SPRITE_TRAIL, 0, customPlace or player.Position, Vector.Zero, nil):ToEffect()
...
function InutilLib.FireGenericProjAttack(ent, variant, subtype, position, velocity)
	local proj = Isaac.Spawn(9, variant, subtype, position, velocity, ent):ToProjectile()
	return proj
end
