...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_EFFECT_UPDATE, function(_, eff)
	...
	elseif data.IsLeftover then
		if eff.FrameCount == 1 then --beginning
			...
			if data.Player:GetHeadColor() == SkinColor.SKIN_WHITE then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/bone/rebekah_bone_wave_white.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_BLACK then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/bone/rebekah_bone_wave_black.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_BLUE then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/bone/rebekah_bone_wave_blue.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_RED then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/bone/rebekah_bone_wave_red.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_GREEN then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/bone/rebekah_bone_wave_green.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_GREY then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/bone/rebekah_bone_wave_grey.png")
			end
			sprite:LoadGraphics()
	...
	elseif data.SummonImmortalBackup then
		if eff.FrameCount == 1 then --beginning
			...
			if data.Player:GetHeadColor() == SkinColor.SKIN_WHITE then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/immortal/rebekah_summon_angelic_prism_white.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_BLACK then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/immortal/rebekah_summon_angelic_prism_black.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_BLUE then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/immortal/rebekah_summon_angelic_prism_blue.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_RED then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/immortal/rebekah_summon_angelic_prism_red.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_GREEN then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/immortal/rebekah_summon_angelic_prism_green.png")
			elseif data.Player:GetHeadColor() == SkinColor.SKIN_GREY then
				sprite:ReplaceSpritesheet(0, "gfx/characters/costumes/immortal/rebekah_summon_angelic_prism_grey.png")
			end
			sprite:LoadGraphics()
			data.Player.Visible = false
			InutilLib.SFX:Play(RebekahCurse.Sounds.SOUND_REDCHARGELIGHT, 1, 0, false, 1)
	...
	elseif data.DashBrokenFragment then
		...
		for i, ent in pairs (Isaac.GetRoomEntities()) do
			...
			elseif ent.Type == 5 then
				local function convertpickup(pickup)
					if pickup.Variant == PickupVariant.PICKUP_COIN and math.random(1,4) == 4 then
					...
					elseif pickup.Variant == PickupVariant.PICKUP_COLLECTIBLE then
						if math.random(1,2) == 2 then
							...
							data.Player:AddCollectible(CollectibleType.COLLECTIBLE_TMTRAINER, 0, false)
							--pickup:Morph(5, PickupVariant.PICKUP_COLLECTIBLE, math.random(0,InutilLib.GetMaxCollectibleID()), false, true, false)
							local coll = Isaac.Spawn(5, PickupVariant.PICKUP_COLLECTIBLE, -1, pickup.Position, Vector(0,0), nil):ToPickup()
							pickup:Remove()
	...
	elseif data.TildeConsole then
		...
		if sprite:IsPlaying("SuccessLoop") and not data.Debug then
			...
			if (data.Player:GetShootingInput().X ~= 0 or data.Player:GetShootingInput().Y ~= 0) then
				data.Player.FireDelay = 60
				data.Player:UseActiveItem(CollectibleType.COLLECTIBLE_DATAMINER, true, false, false, false, -1)
				--debug 1
				...
				if sprite:IsOverlayPlaying("Overlay_4") then
					...
					InutilLib.SetTimer( 120, function()
						yandereWaifu.GetEntityData(data.Player).BrokenBuff = false
						data.Player:AddCacheFlags(CacheFlag.CACHE_DAMAGE);
						data.Player:EvaluateItems()
					end)
				end
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_EFFECT_RENDER, function(_, eff)
	...
	elseif data.DashBrokenFragment then
		if data.IsBlue then
			local customColor = Color(0, 0, 1, 1, 0, 0, 0)
			eff:SetColor(customColor, 9999999999, 1, false, false)
			
		elseif data.IsOrange then
			local customColor = Color(1, 0.5, 0, 1, 0, 0, 0)
			eff:SetColor(customColor, 9999999999, 1, false, false)
		end
	end
end, RebekahCurse.ENTITY_EXTRACHARANIMHELPER)
