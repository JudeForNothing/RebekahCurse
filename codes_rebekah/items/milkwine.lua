yandereWaifu:AddCallback(ModCallbacks.MC_EVALUATE_CACHE, function(_,player, cacheF) --The thing the checks and updates the game, i guess?
	...
	if player:HasCollectible(RebekahCurseItems.COLLECTIBLE_MILKWINE) and data.MilkWineMultiplier then
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_,player)
	local data = yandereWaifu.GetEntityData(player)
	if player:HasCollectible(RebekahCurseItems.COLLECTIBLE_MILKWINE) and InutilLib.HasJustPickedCollectible( player, RebekahCurseItems.COLLECTIBLE_MILKWINE) then
		--player:AddNullCostume(RebekahCurseCostumes.CandyWeddingRing)
	end
    if player:HasCollectible(RebekahCurseItems.COLLECTIBLE_MILKWINE) and player.FrameCount % 15 == 0 then
