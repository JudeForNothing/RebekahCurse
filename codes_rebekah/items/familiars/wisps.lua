function yandereWaifu:rebekahItemWispUpdate(wisp)
	...
	if wisp.Player and wisp.SubType == RebekahCurse.Items.COLLECTIBLE_DOORSTOPPER then
        if wisp.FrameCount >= 60 then
            wisp:Remove()
        end
    end
    if wisp.Player and wisp.SubType == RebekahCurse.Items.COLLECTIBLE_OHIMDIE then
...
function yandereWaifu:rebekahItemWispCollision(wisp, coll, low)
	--[[if wisp.Player and wisp.SubType == RebekahCurse.Items.COLLECTIBLE_OHIMDIE and wisp.HitPoints <= 1 then
...
function yandereWaifu:rebekahItemWispDamage(wisp, damage, damageFlags, coll, damageCountdown)
    if wisp.Type == 3 and wisp.Variant == FamiliarVariant.WISP then
        ...
        if wisp and wisp.Player and wisp.SubType == RebekahCurse.Items.COLLECTIBLE_OHIMDIE and wisp.HitPoints <= 1 then
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_ENTITY_REMOVE, function(_, wisp)
    if wisp.Variant == FamiliarVariant.WISP then
        wisp = wisp:ToFamiliar()
        if wisp.SubType == RebekahCurse.Items.COLLECTIBLE_REBEKAHSCAMERA then
            ...
            InutilLib.SFX:Play( RebekahCurse.Sounds.SOUND_CAMERAUSE, 1.4, 0, false, 1 );
        end
    end
end, 3)