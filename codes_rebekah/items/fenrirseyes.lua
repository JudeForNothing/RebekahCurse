yandereWaifu:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_,player)
    ...
    if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_FENRIRSEYE) then
        local angle = InutilLib.DirToVec(player:GetHeadDirection()):GetAngleDegrees()
        ...
        for _, ent in pairs(ents) do
            if InutilLib.CuccoLaserCollision(player, angle, 700, ent, 50) then
                --ent:AddFear(EntityRef(ent), 10)
                yandereWaifu.GetEntityData(ent).isIntimidated = 15
            end
        end
    end
end)