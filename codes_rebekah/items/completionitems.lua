...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_PLAYER_UPDATE, function(_,player)
	...
	if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_POWERLOVE) and InutilLib.HasJustPickedCollectible( player, RebekahCurse.Items.COLLECTIBLE_POWERLOVE) then
		player:AddNullCostume(LovePower)
	end
	--love = Power
	local H = player:GetHearts()
	if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_POWERLOVE) then
		if data.H ~= H then
		...
		data.H = H
	elseif not player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_POWERLOVE) and data.H then
		data.H = nil
		player:AddCacheFlags(CacheFlag.CACHE_DAMAGE);
		player:AddCacheFlags(CacheFlag.CACHE_SPEED);
		player:EvaluateItems()
	...
	if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_LOVESICK) then
		if InutilLib.HasJustPickedCollectible( player, RebekahCurse.Items.COLLECTIBLE_LOVESICK) then
...
function yandereWaifu:ItemsNewRoom()
	for p = 0, game:GetNumPlayers() - 1 do
		...
		if player:HasTrinket(RebekahCurse.Trinkets.TRINKET_ISAACSLOCKS) and room:IsFirstVisit() then
...
--stat cache for each mode
	function yandereWaifu:itemcacheregister(player, cacheF) --The thing the checks and updates the game, i guess?
		...
		if cacheF == CacheFlag.CACHE_FAMILIARS then
			--Miraculous Womb
			player:CheckFamiliar(RebekahCurse.ENTITY_ORBITALESAU, player:GetCollectibleNum(CRebekahCurse.OLLECTIBLE_MIRACULOUSWOMB), player:GetCollectibleRNG(RebekahCurse.Items.COLLECTIBLE_MIRACULOUSWOMB))
			player:CheckFamiliar(RebekahCurse.ENTITY_ORBITALJACOB, player:GetCollectibleNum(RebekahCurse.Items.COLLECTIBLE_MIRACULOUSWOMB), player:GetCollectibleRNG(RebekahCurse.Items.COLLECTIBLE_MIRACULOUSWOMB))
		end
		--love = power
		if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_POWERLOVE) then
			local maxH, H = player:GetMaxHearts(), player:GetHearts()
