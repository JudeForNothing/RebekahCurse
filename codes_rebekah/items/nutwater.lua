...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_PEFFECT_UPDATE, function(_,player)
	...
	if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_NUTWATER) and InutilLib.HasJustPickedCollectible( player, RebekahCurse.Items.COLLECTIBLE_NUTWATER ) then
		...
		--for i = 0, 1, 1 do
			...
			elseif player:GetPlayerType() == PlayerType.PLAYER_AZAZEL or player:GetPlayerType() == PlayerType.PLAYER_LILITH or player:GetPlayerType() == PlayerType.PLAYER_AZAZEL_B or player:GetPlayerType() == PlayerType.PLAYER_LILITH_B then
				local newpickup = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_HEART, 6, InutilLib.room:FindFreePickupSpawnPosition(player.Position,3), player.Velocity, player)
			elseif player:GetPlayerType() == PlayerType.PLAYER_THEFORGOTTEN then
				local newpickup = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_HEART, 11, InutilLib.room:FindFreePickupSpawnPosition(player.Position,3), player.Velocity, player)
			else
				local tbl = {
					[1] = 3,
					[2] = 8
				}
				local newpickup = Isaac.Spawn(EntityType.ENTITY_PICKUP, PickupVariant.PICKUP_HEART, tbl[math.random(1,2)], InutilLib.room:FindFreePickupSpawnPosition(player.Position,3), player.Velocity, player)
			end
		--end
	end
	if player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_NUTWATER) then
	...
	--[[if InutilLib.room:GetGridEntityFromPos(player.Position) then
		...
		if InutilLib.room:GetGridEntityFromPos(player.Position):GetType() == GridEntityType.GRID_WALL and StageAPI and StageAPI.Loaded then
			if data.GonnaNoClip > 10 then
				...
				player.Velocity = player.Velocity + ((InutilLib.room:GetGridEntityFromPos(player.Position).Position - player.Position):Resized(math.random(1,2))) * 0.7
				InutilLib.game:ShakeScreen(3)
			end
		else
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_NEW_ROOM, function()
	...
	local thematicGen
	local withItemGen
	if (InutilLib.room:GetType() == RoomType.ROOM_SUPERSECRET or InutilLib.room:GetType() == RoomType.ROOM_SECRET) and generateSomeHoles and not yandereWaifu.STAGE.Liminal:IsStage() then
		for k, v in pairs(InutilLib.GetRoomGrids()) do
			if v:GetType() == GridEntityType.GRID_WALL then
				...
				if v:GetRNG():GetSeed() < 100000000 then
					...
					InutilLib.room:SpawnGridEntity(v:GetGridIndex(), GridEntityType.GRID_PIT, 0, 0, 0)
