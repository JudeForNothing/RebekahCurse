...
yandereWaifu:AddCallback(ModCallbacks.MC_ENTITY_TAKE_DMG, function(_, ent, damage, flags, source, countdown)
	...
	if data.TakeDmgCancel ~= true and not cloned and ent:ToNPC() then
		...
		then
            ...
            if player ~= nil then
                if not player:HasCollectible(RebekahCurse.Items.COLLECTIBLE_REGICIDE) then return end
                local rng = Isaac.GetPlayer():GetCollectibleRNG(RebekahCurse.Items.COLLECTIBLE_REGICIDE)
			    if rng:RandomFloat() < chance and not yandereWaifu.GetEntityData(ent).IsGonnaCombo then
                    local secondHandMultiplier = player:GetTrinketMultiplier(TrinketType.TRINKET_SECOND_HAND) + 1
                    yandereWaifu.AddRegicide(ent, 60*secondHandMultiplier)
                    yandereWaifu.AddRegicideCount(ent)
                end
            end
