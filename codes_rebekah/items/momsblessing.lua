...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_PICKUP_UPDATE, function(_, pickup)
	...
	for p = 0, ILIB.game:GetNumPlayers() - 1 do
		...
		if (player:HasTrinket(RebekahCurseTrinkets.TRINKET_MOMSBLESSING)) then
			if rng:RandomFloat() <= (chance) and validPickup and ILIB.room:GetType() ~= RoomType.ROOM_BOSS and RebekahCurseGlobalData.MOMS_BLESSING_NO_MORPH_FRAME == 0 and (pickup:GetSprite():IsPlaying("Appear") or pickup:GetSprite():IsPlaying("AppearFast")) and pickup:GetSprite():GetFrame() == 1 and not pickup.SpawnerEntity then
...
yandereWaifu:AddCallback(ModCallbacks.MC_POST_UPDATE, function()
	if RebekahCurseGlobalData.MOMS_BLESSING_NO_MORPH_FRAME > 0 and ILIB.game:GetFrameCount() > RebekahCurseGlobalData.MOMS_BLESSING_NO_MORPH_FRAME + 1 then --set frame back to zero
